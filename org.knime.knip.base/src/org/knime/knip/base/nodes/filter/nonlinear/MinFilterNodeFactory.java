 ...
 */
public class MinFilterNodeFactory<T extends RealType<T>> extends SlidingWindowOperationNodeFactory<T, T> {
    ...
    protected void createNodeDescription(final KnimeNodeDocument doc) {
        ...
        node.setName("Min Filter");
        node.setShortDescription("Applys Min filtering to images in n-dimensions");
        final FullDescription desc = node.addNewFullDescription();
        desc.addNewIntro().addNewP().newCursor().setTextValue("Applys Min filtering to images in n-dimensions");
    }
    ...
    public AbstractSlidingWindowOperationNodeModel<T, T> createNodeModel() {
        return new AbstractSlidingWindowOperationNodeModel<T, T>() {
            ...
            protected UnaryOutputOperation<ImgPlus<T>, ImgPlus<T>>
                ...
                return defaultUnary(new Min<T, T>(), type, neighborhood, outStrat);
            }
