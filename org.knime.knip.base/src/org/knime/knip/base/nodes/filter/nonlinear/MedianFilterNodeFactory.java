...
public class MedianFilterNodeFactory<T extends RealType<T>> extends SlidingWindowOperationNodeFactory<T, T> {
    ...
    protected void createNodeDescription(final KnimeNodeDocument doc) {
        ...
        node.setName("Median Filter");
        node.setShortDescription("Applys Median filtering to images in n-dimensions");
        final FullDescription desc = node.addNewFullDescription();
        desc.addNewIntro().addNewP().newCursor().setTextValue("Applys Median filtering to images in n-dimensions");
    }
