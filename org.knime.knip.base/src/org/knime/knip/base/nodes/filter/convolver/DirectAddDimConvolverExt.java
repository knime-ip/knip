...
public class DirectAddDimConvolverExt<T extends RealType<T>, O extends RealType<O> & NativeType<O>, K extends RealType<K> & NativeType<K>>
    ...
    public void addNodeDescription(final Tab desc) {
        ...
        opt.setName("Direct Convolution (Add Dimension)");
        opt.addNewP()
                .newCursor()
                .setTextValue("Direct convolution of the image. Convolution takes place in image space. Each convolution is applied on the input image individually. The results are stored in an additional dimension.");
    }
    ...
    public ImgPlus<O> compute(final ImgPlus<T> input, final Img<K>[] kernels, final ImgPlus<O> output) {
        ...
        final AdditionDimImgConvolver<T, K, O> convolver =
                new AdditionDimImgConvolver<T, K, O>(new DirectConvolver<T, K, O>());
    ...
    public String getName() {
        return "Direct Convolver (Add Dimension)";
    }
