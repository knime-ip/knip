...
public class ImgWriter {
    ...
    private void retrieveSupportedWriters() {
        ...
        if (m_mapWriters == null) {
            ...
            final ClassList<IFormatWriter> defaultClasses =
     ...
     * @param writer the writer
     * @param compressionType the compression type, if available, can be
     *            <code>null</code>.
     * @param dimMapping mapping of the image dimensions (without X and Y) to
     *            the dimension order ZCT (array must be of length 3), If
    ...
    public <T extends RealType<T>> void writeImage(final Img<T> img,
        ...
        final int sizeZ =
                ((img.numDimensions() > map[0]) && (map[0] != -1)) ? (int)img
                        .dimension(2 + map[0]) : 1;
        int sizeC =
                (img.numDimensions() > map[1]) && (map[1] != -1) ? (int)img
                        .dimension(2 + map[1]) : 1;
        if (sizeC > 3) {
        ...
        final int sizeT =
                (img.numDimensions() > map[2]) && (map[2] != -1) ? (int)img
                        .dimension(2 + map[2]) : 1;
        ...
        final boolean littleEndian =
                !writer.getMetadataRetrieve().getPixelsBinDataBigEndian(0, 0)
                        .booleanValue();

        final IntervalIterator fakeCursor =
                new IntervalIterator(new int[]{sizeZ, sizeT});
        ...
        while (fakeCursor.hasNext()) {
            ...
            for (int i = 0; i < sizeC; i++) {
                ...
                final long[] zctPos =
                        new long[]{fakeCursor.getLongPosition(0), i,
                ...
                if (val instanceof ByteType) {
                    planes[i] =
                            new byte[(int)(img.dimension(0) * img.dimension(1))];
                    while (c.hasNext()) {
                        c.fwd();
                        planes[i][c.getIntPosition(0)
                                + ((int)img.dimension(0) * c.getIntPosition(1))] =
                                ((ByteType)c.get()).get();
                    }
                    ...
                    final short[] tmp =
                            new short[(int)(img.dimension(0) * img.dimension(1))];
                    while (c.hasNext()) {
                        c.fwd();
                    ...
                    final float[] tmp =
                            new float[(int)(img.dimension(0) * img.dimension(1))];
                    while (c.hasNext()) {
                        c.fwd();
                        tmp[c.getIntPosition(0)
                                + ((int)img.dimension(0) * c.getIntPosition(1))] =
            ...
            final int index =
                    FormatTools.getIndex(DimensionOrder.XYZCT.toString(),
                            sizeZ, 1, sizeT, numSteps,
