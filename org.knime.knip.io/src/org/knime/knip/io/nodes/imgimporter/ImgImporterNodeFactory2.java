 ...
 *         Zinsmaier</a>
 */
public class ImgImporterNodeFactory2<T extends RealType<T> & NativeType<T>>
	...
	private static SettingsModelSubsetSelection2 createSubsetSelectionModel() {
		return new SettingsModelSubsetSelection2("subsetselection");
	}
	...
	public ValueToCellNodeModel<ImgRefValue, ImgPlusCell<T>> createNodeModel() {
		return new ValueToCellNodeModel<ImgRefValue, ImgPlusCell<T>>() {
			...
			private final SettingsModelSubsetSelection2 m_subsetSelect = createSubsetSelectionModel();
			...
			@Override
			protected ImgPlusCell<T> compute(final ImgRefValue cellValue) throws Exception {
				...
				return m_imgCellFactory.createCell((ImgPlus<T>) imgSource.getImg(ref, 0));
			}
