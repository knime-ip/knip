...
public class MaximumFinder implements ExtendedPlugInFilter, DialogListener {
    ...
    public void run(ImageProcessor ip) {
        ...
        outIp = findMaxima(ip, tolerance, threshold, outputType, excludeOnEdges, false); //process the image, get all! local maxima sorted
        if (outIp == null)
         {
     ...
     */
    public Polygon getMaxima(final ImageProcessor ip, final double tolerance, final boolean excludeOnEdges) {
     ...
     */
    public ByteProcessor findMaxima(final ImageProcessor ip, final double tolerance, double threshold,
            final int outputType, final boolean excludeOnEdges, final boolean isEDM) {
        if (dirOffset == null) {
     ...
     */
    long[] getSortedMaxPoints(final ImageProcessor ip, final ByteProcessor typeP, final boolean excludeEdgesNow,
            final boolean isEDM, final float globalMin, final float globalMax, final double threshold) {
        Rectangle roi = ip.getRoi();
        byte[] types =  (byte[])typeP.getPixels();
    ...
    * @param outputType
    */
   void analyzeAndMarkMaxima(final ImageProcessor ip, final ByteProcessor typeP, final long[] maxPoints, final boolean excludeEdgesNow,
        final boolean isEDM, final float globalMin, final double tolerance, final int outputType, final float maxSortingError) {
        byte[] types =  (byte[])typeP.getPixels();
    ...
    */
    ByteProcessor make8bit(final ImageProcessor ip, final ByteProcessor typeP, final boolean isEDM, final float globalMin, final float globalMax, double threshold) {
        byte[] types = (byte[])typeP.getPixels();
        double minValue;
        if (isEDM) {
            threshold = 0.5;
   ...
   /** Get estimated "true" height of a maximum or saddle point of a Euclidian Distance Map.
     ...
     */
    float trueEdmHeight(final int x, final int y, final ImageProcessor ip) {
        int xmax = width - 1;
     ...
     */
    void cleanupMaxima(final ByteProcessor outIp, final ByteProcessor typeP, final long[] maxPoints) {
        byte[] pixels = (byte[])outIp.getPixels();
        byte[] types = (byte[])typeP.getPixels();
        int nMax = maxPoints.length;
     ...
     */
    void cleanupExtraLines(final ImageProcessor ip) {
    ...
    void removeLineFrom (final byte[] pixels, int x, int y) {
     ...
     */
    int nRadii (final byte[] pixels, final int x, final int y) {
        int offset = x + y*width;
     ...
     */
    private void watershedPostProcess(final ImageProcessor ip) {
     ...
     */
    void deleteEdgeParticles(final ByteProcessor ip, final ByteProcessor typeP) {
        byte[] pixels = (byte[])ip.getPixels();
        byte[] types = (byte[])typeP.getPixels();
        width = ip.getWidth();
        height = ip.getHeight();
     ...
     */
    void deleteParticle(final int x, final int y, final ByteProcessor ip, final Wand wand) {
     ...
     */
    private boolean watershedSegment(final ByteProcessor ip) {
     ...
     */
    private int processLevel(final int pass, final ImageProcessor ip, final int[] fateTable,
            final int levelStart, final int levelNPoints, final int[] coordinates, final int[] setPointList) {
        int xmax = width - 1;
        int ymax = height - 1;
        byte[] pixels = (byte[])ip.getPixels();
     ...
     */
    void makeDirectionOffsets(final ImageProcessor ip) {
        width = ip.getWidth();
        height = ip.getHeight();
        int shift = 0, mult=1;
        do {
     ...
     */
    boolean isWithin(final int x, final int y, final int direction) {
        int xmax = width - 1;
        int ymax = height -1;
        switch(direction) {
            case 0:
    ...
    private void addProgress(final double deltaProgress) {
        if (nPasses==0) {
            return;
        }
