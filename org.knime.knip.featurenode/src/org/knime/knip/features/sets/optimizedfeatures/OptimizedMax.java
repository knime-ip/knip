 ...
 *            output type
 */
@Plugin(type = Max.class, label = "Statistics: Max", priority = Priority.FIRST_PRIORITY - 1)
public class OptimizedMax<IN extends RealType<IN>, OUT extends RealType<OUT>>
		extends AbstractFunctionOp<Iterable<IN>, OUT> implements Max {
	...
	public OUT compute(Iterable<IN> input) {
		return (OUT) function.compute(input).getB();
	}
}