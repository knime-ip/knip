...
public class ImgComparatorNodeModel<T extends NativeType<T> & RealType<T>> extends
    ...
    @Override
    protected void compare(final ImgPlusValue<T> vin1, final ImgPlusValue<T> vin2) {
        final ImgPlus<T> img1 = vin1.getImgPlus();
        final ImgPlus<T> img2 = vin2.getImgPlus();
        final Cursor<T> c1 = img1.cursor();
        final Cursor<T> c2 = img2.cursor();

        while (c1.hasNext()) {
            c1.fwd();
            c2.fwd();
            if (Double.compare(c1.get().getRealDouble(), c2.get().getRealDouble()) != 0) {
                throw new IllegalStateException("Two images are not the same! " + img1.getSource() + " "
                        + img1.getName() + " is not the same as " + img2.getSource() + " " + img2.getName());
            }
        }
    }
}
